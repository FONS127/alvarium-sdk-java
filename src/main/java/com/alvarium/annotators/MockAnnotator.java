...
class MockAnnotator implements Annotator {
  ...
  public Annotation execute(PropertyBag ctx, byte[] data) throws AnnotatorException {
    ...
    try {
      ...
      final String imageTag = System.getenv(AbstractAnnotator.IMAGE_TAG_ENV_KEY) == null ? "" : System.getenv(AbstractAnnotator.IMAGE_TAG_ENV_KEY);
      final String sig = signature.getPublicKey().getType().toString();

      final Annotation annotation = new Annotation(key, hash, host, imageTag, kind, sig, cfg.getShouldSatisfy(), Instant.now());
      return annotation;
